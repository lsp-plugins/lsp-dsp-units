...
namespace lsp
{
    namespace dspu
    {
        namespace rt
        {
            ...
            status_t mesh_t::add_object(Object3D *obj, ssize_t oid, const dsp::matrix3d_t *transform, rt::material_t *material)
            {
                // Reset tags
                ...
                for (size_t i=0, n=obj->num_triangles(); i<n; ++i)
                {
                    ...
                    rtm::triangle_t *dt = triangle.alloc();
                    if (dt == NULL)
                    ...
                    {
                        // Allocate vertex
                        rtm::vertex_t *vx   = reinterpret_cast<rtm::vertex_t *>(st->v[j]->ptag);
                        if (st->v[j]->ptag == NULL)
                        {
                            vx              = vertex.alloc();
                            if (vx == NULL)
                        ...
                        // Allocate edge
                        rtm::edge_t *ex     = reinterpret_cast<rtm::edge_t *>(st->e[j]->ptag);
                        if (ex == NULL)
                        {
                            ex              = edge.alloc();
                            if (ex == NULL)
                ...
                {
                    rtm::edge_t *de      = edge.get(i);
                    obj_edge_t *se      = reinterpret_cast<obj_edge_t *>(de->ptag);

                    de->v[0]            = reinterpret_cast<rtm::vertex_t *>(se->v[0]->ptag);
                    de->v[1]            = reinterpret_cast<rtm::vertex_t *>(se->v[1]->ptag);
                }

                // Patch triangle structures and link to edges
                for (size_t i=start_t, n=triangle.size(); i<n; ++i)
                {
                    rtm::triangle_t *dt = triangle.get(i);
                    obj_triangle_t *st  = reinterpret_cast<obj_triangle_t *>(dt->ptag);

                    dt->v[0]            = reinterpret_cast<rtm::vertex_t *>(st->v[0]->ptag);
                    dt->v[1]            = reinterpret_cast<rtm::vertex_t *>(st->v[1]->ptag);
                    dt->v[2]            = reinterpret_cast<rtm::vertex_t *>(st->v[2]->ptag);

                    dt->e[0]            = reinterpret_cast<rtm::edge_t *>(st->e[0]->ptag);
                    dt->e[1]            = reinterpret_cast<rtm::edge_t *>(st->e[1]->ptag);
                    dt->e[2]            = reinterpret_cast<rtm::edge_t *>(st->e[2]->ptag);
            ...
            bool mesh_t::unlink_triangle(rtm::triangle_t *t, rtm::edge_t *e)
            {
                for (rtm::triangle_t **pcurr = &e->vt; *pcurr != NULL; )
                {
                    rtm::triangle_t *curr   = *pcurr;
                    rtm::triangle_t **pnext = (curr->e[0] == e) ? &curr->elnk[0] :
            ...
            status_t mesh_t::arrange_triangle(rtm::triangle_t *ct, rtm::edge_t *e)
            {
                rtm::vertex_t *tv;
                rtm::edge_t *te;
                rtm::triangle_t *tt;
            ...
            status_t mesh_t::split_edge(rtm::edge_t* e, rtm::vertex_t* sp)
            {
                status_t res;
                rtm::triangle_t *ct, *nt, *pt;
                rtm::edge_t *ne, *se;
            ...
            status_t mesh_t::split_triangle(rtm::triangle_t* t, rtm::vertex_t* sp)
            {
            ...
            status_t mesh_t::solve_conflicts()
            {
                status_t res;
                dsp::vector3d_t pl;
                dsp::vector3d_t spl[3]; // Scissor planes
                size_t tag;

                // Intialize tags
                RT_FOREACH(rtm::edge_t, e, edge)
                    e->itag     = 0;
                RT_FOREACH_END;

                for (size_t i=0; i<triangle.size(); )
                {
                    rtm::triangle_t *ct  = triangle.get(i++);
                    ct->itag            = i;
                }
            ...
            ssize_t mesh_t::linked_count(rtm::triangle_t *t, rtm::edge_t *e)
            {
                if ((t == NULL) || (e == NULL))
                    return -1;

                size_t n = 0;
                for (rtm::triangle_t *p = e->vt; p != NULL; )
                {
            ...
            bool mesh_t::validate_list(rtm::edge_t *e)
            {
                rtm::triangle_t *t = e->vt;
                size_t n = 0;
...
//                    lsp_trace("Edge has no link with triangle");
            ...
            status_t mesh_t::copy(mesh_t *src)
            {
                Allocator3D<rtm::vertex_t>   vertex(1024);
                Allocator3D<rtm::edge_t>     edge(1024);
                Allocator3D<rtm::triangle_t> triangle(1024);

                // Clone vertexes
                RT_FOREACH(rtm::vertex_t, sv, src->vertex)
                    rtm::vertex_t *dv = vertex.alloc(sv);
                    if (dv == NULL)
                        return STATUS_NO_MEM;
                ...
                // Clone edges
                RT_FOREACH(rtm::edge_t, se, src->edge)
                    rtm::edge_t *de = edge.alloc(se);
                    if (de == NULL)
                        return STATUS_NO_MEM;
                ...
                // Clone triangles
                RT_FOREACH(rtm::triangle_t, st, src->triangle)
                    rtm::triangle_t *dt = triangle.alloc(st);
                    if (dt == NULL)
                        return STATUS_NO_MEM;
                ...
                // Patch edge structures and link to vertexes
                RT_FOREACH(rtm::edge_t, ex, edge)
                    rtm::edge_t *se   = reinterpret_cast<rtm::edge_t *>(ex->ptag);
                    if (se == NULL) // Edge does not need patching
                        continue;

                    // Patch vertex pointers if needed
                    ex->v[0]        = (se->v[0]->ptag != NULL) ? reinterpret_cast<rtm::vertex_t *>(se->v[0]->ptag) : se->v[0];
                    ex->v[1]        = (se->v[1]->ptag != NULL) ? reinterpret_cast<rtm::vertex_t *>(se->v[1]->ptag) : se->v[1];
                RT_FOREACH_END

                // Link triangle structures to edges
                RT_FOREACH(rtm::triangle_t, tx, triangle)
                    rtm::triangle_t *st  = reinterpret_cast<rtm::triangle_t *>(tx->ptag);
                    if (st == NULL) // Triangle does not need patching
                        continue;

                    tx->v[0]            = reinterpret_cast<rtm::vertex_t *>(st->v[0]->ptag);
                    tx->v[1]            = reinterpret_cast<rtm::vertex_t *>(st->v[1]->ptag);
                    tx->v[2]            = reinterpret_cast<rtm::vertex_t *>(st->v[2]->ptag);

                    tx->e[0]            = reinterpret_cast<rtm::edge_t *>(st->e[0]->ptag);
                    tx->e[1]            = reinterpret_cast<rtm::edge_t *>(st->e[1]->ptag);
                    tx->e[2]            = reinterpret_cast<rtm::edge_t *>(st->e[2]->ptag);
            ...
            bool mesh_t::validate()
            {
                for (size_t i=0, n=vertex.size(); i<n; ++i)
                {
                    rtm::vertex_t *v      = vertex.get(i);
                    if (v == NULL)
                        return false;
                }
                ...
                for (size_t i=0, n=triangle.size(); i<n; ++i)
                {
                    rtm::triangle_t *t    = triangle.get(i);
                    if (t == NULL)
                        return false;
